...
export default function GridCard2({
  ...
  const displayTitle = extractedTitle?.includes(":")
    ? extractedTitle.split(":", 2)[1]?.trim()
    : extractedTitle;
  ...
  const CardContent = () => {
    ...
    // Default: image background, text centered horizontally only
    const isBento = safeBentoVariant !== "none";
    return (
      <div
        className={cn(
      ...
      >
        ...
        <div className="relative z-10 flex h-full w-full flex-col justify-end p-4">
          <div className="transform transition-transform duration-300 group-hover:translate-y-[-4px] w-full text-center">
            {displayTitle && (
              <p className="text-lg font-bold leading-tight text-primary md:text-xl">
                {displayTitle}
              </p>
            )}
          </div>
